 ...
 * @private
 */
Algorithm.prototype._count = function(methodName) {
  var id = this.identifier + '_' + methodName;
  Profile.count(id);
};
...
Algorithm.prototype.check = function(expected) {
	...
	if (args.indexOf('--p') > -1) {
		...
		sender.send('node-algorithms', this.identifier + ' -> ' + this.total, 1);
	}
 ...
 * @public
 */
Algorithm.prototype.readFileByLine = function(filename, onLineRead, onClose) {
	...
	if (typeof onLineRead === 'function') {
		rd.on('line', onLineRead);
	}
	...
	return {
		onEach: function(onEach) {
			...
			if (self.value(params).isArray()) {
				...
				if (!params.reverse) {
					for (i = params.start; i < params.limit; i++) {
