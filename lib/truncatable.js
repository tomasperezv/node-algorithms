...
Truncatable.prototype.solve = function(integer) {
  ...
  var prefix = parseInt(stringify.substring(0, 2));
  var suffix = parseInt(stringify.substring(stringify.length-2));

  if (!new Prime().solve(prefix) || !new Prime().solve(suffix)) {
    return false;
  } else if (integer < 10 && !(new Prime().solve(integer))) {
    return false;
  } else {
